...
function exibirQuizz(resposta) {
    ...
    let quizzesPublicos = document.querySelector(
        ".todosQuizzes .containerQuizzes"
    );
    let quizzesDoUsuario = document.querySelector(
        ".seusQuizzes .containerQuizzes"
    );
...
function selecionarQuizz(quizzid) {
    const promise = axios.get(
        `https://mock-api.driven.com.br/api/vm/buzzquizz/quizzes/${quizzid}`
    );
...
function reiniciarQuiz() {
    ...
    let selecionado = document.querySelectorAll(
        `.container-perguntas. .selecionar-questao span`
    );
    let desabilitarClick = document.querySelectorAll(
        `.container-perguntas. .selecionar-questao `
    );
...
function verificar(resposta, verdadeiroOuFalso) {
    let selecionado = document.querySelectorAll(
        `.container-perguntas.id-${[contador]} .selecionar-questao span`
    );
    let desabilitarClick = document.querySelectorAll(
        `.container-perguntas.id-${[contador]} .selecionar-questao `
    );
...
function irParaQuizz(resposta) {
    ...
    for (let i = 0; i < quizSelecionado.questions.length; i++) {
        ...
        perguntas.innerHTML += `
            <div data-test="question" onclick="selecionar(this)" class="container-perguntas  id-${[
                i,
            ]}">
                <div id="mudaCor${[
                    i,
                ]}" data-test="question-title" class="titulo-pergunta">${
            quizSelecionado.questions[i].title
        }</div>
                <div class="caixa-questoes">
                    <div data-test="answer" onclick="verificar(this, ${
                        sortearRespostas[0].isCorrectAnswer
                    })" class="selecionar-questao">
                        <img  src="${sortearRespostas[0].image}">
                        <span data-test="answer-text" data-verificar="${
                            sortearRespostas[0].isCorrectAnswer
                        }">${sortearRespostas[0].text}</span>
                    </div>
                    <div  data-test="answer" onclick="verificar(this, ${
                        sortearRespostas[1].isCorrectAnswer
                    })" class="selecionar-questao">
                        <img  src="${sortearRespostas[1].image}">
                        <span data-test="answer-text" data-verificar="${
                            sortearRespostas[1].isCorrectAnswer
                        }">${sortearRespostas[1].text}</span>
                    </div>
                    <div data-test="answer" onclick="verificar(this, ${
                        sortearRespostas[2].isCorrectAnswer
                    })"  class="selecionar-questao">
                        <img src="${sortearRespostas[2].image}">
                        <span data-test="answer-text" data-verificar="${
                            sortearRespostas[2].isCorrectAnswer
                        }">${sortearRespostas[2].text}</span>
                    </div>
                    <div data-test="answer" onclick="verificar(this, ${
                        sortearRespostas[3].isCorrectAnswer
                    })" class="selecionar-questao">
                        <img src="${sortearRespostas[3].image}">
                        <span data-test="answer-text" data-verificar="${
                            sortearRespostas[3].isCorrectAnswer
                        }">${sortearRespostas[3].text}</span>
                    </div>
            </div>
        `;
        document.getElementById(`mudaCor${[i]}`).style.backgroundColor =
            quizSelecionado.questions[i].color;
    }
...
function CriarPerguntas() {
    ...
    if (
        ...
        pag3_2.innerHTML += `<div class="BotaoVermelho Tela3_ParaNives" data-test="go-create-levels" onclick="CriarNives()">Prosseguir pra criar níveis</div>`;
        pag3_3.innerHTML += ` <div class="BotaoVermelho Tela3_FinalizarQuizz" data-test="finish" onclick="AddQuizz()">Finalizar Quizz</div>`;
        //*cria a pagina 3-4
        pag3_4.innerHTML += `
...
function CriarNives() {
    ...
    let URLrespostaCorreta =
        document.querySelector(`.URLrespostaCorreta0`).value;
    let respostaIncorreta_1 = document.querySelector(
        `.respostaIncorreta0_1`
    ).value;
    let URLrespostaIncorreta_1 = document.querySelector(
        `.URLrespostaIncorreta0_1`
    ).value;
    let respostaIncorreta_2 = document.querySelector(
        `.respostaIncorreta0_2`
    ).value;
    let URLrespostaIncorreta_2 = document.querySelector(
        `.URLrespostaIncorreta0_2`
    );
    let respostaIncorreta_3 = document.querySelector(
        `.respostaIncorreta0_3`
    ).value;
    let URLrespostaIncorreta_3 = document.querySelector(
        `.URLrespostaIncorreta0_3`
    ).value;
    ...
    if (
        ...
        for (let c = 0; c < numeroptg; c++) {
            ...
            respostaCorreta = document.querySelector(
                `.respostaCorreta${c}`
            ).value;
            URLrespostaCorreta = document.querySelector(
                `.URLrespostaCorreta${c}`
            ).value;
            respostaIncorreta_1 = document.querySelector(
                `.respostaIncorreta${c}_1`
            ).value;
            URLrespostaIncorreta_1 = document.querySelector(
                `.URLrespostaIncorreta${c}_1`
            ).value;
            respostaIncorreta_2 = document.querySelector(
                `.respostaIncorreta${c}_2`
            ).value;
            URLrespostaIncorreta_2 = document.querySelector(
                `.URLrespostaIncorreta${c}_2`
            ).value;
            respostaIncorreta_3 = document.querySelector(
                `.respostaIncorreta${c}_3`
            ).value;
            URLrespostaIncorreta_3 = document.querySelector(
                `.URLrespostaIncorreta${c}_3`
            ).value;
...
function AddQuizz() {
    ...
    for (let i = 0; i < numeronivel; i++) {
        ...
        let porcentagemNivel = document.querySelector(
            `.porcentagemNivel${i}`
        ).value;
        let imagemNivel = document.querySelector(`.imagemNivel${i}`).value;
        let descricaoNivel = document.querySelector(
            `.descricaoNivel${i}`
        ).value;
        ...
        if (
            ...
            !imagemValida(imagemNivel) ||
            descricaoNivel.legnth < 30 ||
            temZero == 0 ||
            porcentagemNivel < 0 ||
